...
REPLANNER_PROMPT = ChatPromptTemplate.from_template(
    ...
    - action이 Response인 경우, response 필드에는 오직 사용자에게 그대로 보여줄
      ...
      - 출처를 사용한 문장/수치 바로 옆에는 반드시 [1], [2]처럼 번호를 매긴 인용 표시를
        붙이세요. 같은 출처를 여러 번 인용할 때는 같은 번호를 재사용하세요.
...
도구를 호출할 때 query 인자는 검색 엔진에 바로 입력할 짧고 구체적인 키워드/구문으로
작성하세요 (예: "GPT-5.6 공식 발표", "OpenAI ChatGPT 신규 기능"). "~한 사례를 찾는 방법",
"~에 대한 자료" 처럼 검색 방법론이나 의도를 설명하는 문장으로 만들지 마세요 - 찾고자 하는
대상 자체를 검색어로 쓰세요.
