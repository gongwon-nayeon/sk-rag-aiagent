...
def setup_retriever():
    ...
    retriever_tool = create_retriever_tool(
        parent_retriever,
        "retrieve_AI_brief",
        "dataset 폴더의 AI 관련 문서 전체에서 정보를 검색하고 반환합니다. 이 문서는 "
        "'OO년 O월호'처럼 매월 발간되는 월간 보고서로, 매 호마다 그 달에 새로 나온/업데이트된 "
        "AI 모델을 출시일과 함께 정리한 신규 모델 타임라인 섹션을 포함합니다. "
        "이 검색은 문서 제목이나 파일명이 아니라 각 페이지 내용(문장)에 대한 벡터 유사도로 동작합니다. "
        "query 인자는 실제로 문서 본문에 등장할 법한 구체적인 키워드/개체명/주제로 작성하세요 "
        "(예: 'OpenAI ChatGPT 신규 기능', 'GPT-5.6 벤치마크 성능', '2026년 8월 신규 모델 타임라인'). "
